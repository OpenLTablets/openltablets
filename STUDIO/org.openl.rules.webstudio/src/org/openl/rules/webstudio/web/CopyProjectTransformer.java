...
public class CopyProjectTransformer implements ResourceTransformer {
    ...
    public List<FileItem> transformChangedFiles(AProjectFolder projectFolder, List<FileItem> changes) {
        Optional<FileItem> tagsFile = changes.stream().filter(fileItem -> fileItem.getData().getName().equals(ProjectTags.TAGS_FILE_NAME)).findFirst();
        if (!tags.isEmpty() || tagsFile.isPresent()) {
            ...
            try (ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream()) {
                ...
                String fullName = projectFolder.getFolderPath() + "/" + ProjectTags.TAGS_FILE_NAME;
